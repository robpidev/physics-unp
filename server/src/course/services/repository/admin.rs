...
SELECT
student.id AS id,
ocupated, student.names + ' ' + student.last_name1 + ' ' + student.last_name2 AS name
FROM groups
WHERE type::thing("course", $course_id)<-enrolled<-student;
    "#;
    ...
    let students = match resp.take::<Vec<StudentInfoDB>>(0) {
        Ok(s) => s,
        Err(e) => return Err((500, format!("DB Error: {}", e.to_string()))),
    };
